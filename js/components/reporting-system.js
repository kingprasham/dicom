...
async checkAndShowReportStatus() {
    ...
    async saveReportToServer(isAutoSave = false) {
        ...
        const reportData = {
            ...
            studyInstanceUID: currentImage.study_instance_uid,
            seriesInstanceUID: currentImage.series_instance_uid,
        ...
        console.log('Saving report data with Study UID:', reportData);
    ...
    async checkCurrentImageForReports() {
        ...
        try {
            const response = await fetch(`check_report.php?imageId=${currentImage.id}&studyUID=${currentImage.study_instance_uid}`, {
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
                    'Cache-Control': 'no-cache'
            ...
            if (result && result.success && result.exists) {
                if (viewReportBtn) {
                    ...
                    console.log('✓ View Report button shown - report exists');
                }
            } else {
                if (viewReportBtn) {
                    viewReportBtn.style.display = 'none';
                    console.log('✗ View Report button hidden - no report');
                }
            }
        } catch (error) {
